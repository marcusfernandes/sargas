...
function Profile() {
  ...
  const { updateUser, getUser, storeSignature, updatePassword } = useUser();
    ...
    if (await checkValidation(editRef, data, profileValidation)) {
      ...
      if(data.signature){
        if (await checkValidation(editRef, data, signatureValidation)) {
          const file = await storeSignature(data.signature);
      ...
      <BoxContent>
        <Content>
          <div>
            ...
            <Form ref={editRef} initialData={signedUser} onSubmit={handleUser}>
              <Input
                type="text"
                name="username"
                placeholder="Login"
